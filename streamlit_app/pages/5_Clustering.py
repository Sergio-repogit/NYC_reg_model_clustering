...
if not CLUSTERED_DATA.exists():
    st.warning("El archivo final_processed_data.csv no existe. Ejecute el pipeline maestro primero.")
    st.stop()

df_clustered = pd.read_csv(CLUSTERED_DATA)

if 'cluster' not in df_clustered.columns:
    st.warning("Los datos están procesados pero no contienen la columna 'cluster'. Ejecute la fase de clustering para generarlos.")
    st.stop()
...
st.markdown("Promedios de variables clave y análisis modal para interpretar la identidad de cada segmento de mercado.")
...
for cluster_id, group in df_clustered.groupby('cluster'):
    ...
    profile_data.append({
        ...
        'Average Price': round(group['price'].mean(), 2) if 'price' in group.columns else None,
        'Average Distance to Center': round(group['dist_times_square'].mean(), 2) if 'dist_times_square' in group.columns else None,
...
st.markdown("Mapa interactivo de la ciudad donde los colores representan la pertenencia a un cluster específico.")
