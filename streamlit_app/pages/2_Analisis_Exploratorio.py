...
if df is not None:
    ...
    with col_price1:
        min_price = st.number_input("Precio mín. ($):", min_value=0, value=0, step=10)
    with col_price2:
        max_price = st.number_input(
            "Precio máx. ($):", min_value=0, value=int(df["price"].max()), step=10
        )

    # Mínimo de noches
    col_nights1, col_nights2 = st.sidebar.columns(2)
    with col_nights1:
        min_nights = st.number_input("Mín. noches:", min_value=0, value=0, step=1)
    with col_nights2:
        max_nights = st.number_input("Máx. noches:", min_value=0, value=365, step=10)
    ...
    **{len(df_filtered):,}** cumplen tus criterios de búsqueda ({len(df_filtered) / len(df) * 100:.1f}%).
    ...
    if len(df_filtered) == 0:
        st.warning(
            "️ No hay resultados con los filtros seleccionados. Intenta ajustar los parámetros."
        )
        ...
        with tab2:
            ...
            fig_scatter2.update_traces(marker=dict(size=7, opacity=0.6))
        ...
        with tab3:
            ...
            with col_stats:
                ...
                with col_s2:
                    ...
                    st.metric(
                        "Disponibilidad prom.",
                        f"{df_filtered['availability_365'].mean():.0f} días",
                    )
                ...
                df_district = df_district[
                    df_district["room_type"].isin(selected_room_types)
                ]
