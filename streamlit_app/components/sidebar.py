    ...
    if "price" in df.columns:
        ...
        with col1:
            filters["min_price"] = st.number_input(
                "Mínimo ($):", min_value=0, value=0, step=10
            )
        with col2:
            filters["max_price"] = st.number_input(
                "Máximo ($):", min_value=0, value=int(df["price"].max()), step=10
            )
...
def apply_filters(df: pd.DataFrame, filters: dict) -> pd.DataFrame:
    ...
    if "neighbourhood_group" in filters and filters["neighbourhood_group"]:
        df_filtered = df_filtered[
            df_filtered["neighbourhood_group"] == filters["neighbourhood_group"]
        ]

    if "neighbourhood" in filters and filters["neighbourhood"]:
        df_filtered = df_filtered[
            df_filtered["neighbourhood"] == filters["neighbourhood"]
        ]
    ...
    if "min_nights" in filters:
        df_filtered = df_filtered[
            df_filtered["minimum_nights"] >= filters["min_nights"]
        ]

    if "min_reviews" in filters:
        df_filtered = df_filtered[
            df_filtered["number_of_reviews"] >= filters["min_reviews"]
        ]

    if "min_availability" in filters:
        df_filtered = df_filtered[
            df_filtered["availability_365"] >= filters["min_availability"]
        ]
...
def render_sidebar_download(df: pd.DataFrame, filename: str = "data"):
    ...
    st.sidebar.download_button(
        label=" Descargar CSV", data=csv, file_name=f"{filename}.csv", mime="text/csv"
    )
