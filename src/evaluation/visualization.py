...
from utils.config import (
    FIGURES_EDA_DIR, FIGURES_PREPROCESSING_DIR, 
    FIGURES_CLUSTERING_DIR, FIGURES_MODELS_DIR
)
...
def plot_numerical_eda(df: pd.DataFrame, columns: list):
    ...
    for col in columns:
        ...
        fig, (ax_box, ax_hist) = plt.subplots(2, sharex=True, gridspec_kw={"height_ratios": (.15, .85)}, figsize=(10, 8))
...
def plot_pca_biplot(X_scaled, pca, directory: Path):
    ...
    for i, feature in enumerate(features):
        plt.arrow(0, 0, coords[i, 0], coords[i, 1], color='r', alpha=0.5, head_width=0.02)
        plt.text(coords[i, 0]*1.15, coords[i, 1]*1.15, feature, color='g', ha='center', va='center')
...
def plot_pca_variance(pca, directory: Path):
    ...
    plt.step(range(1, len(cum_var) + 1), cum_var, where='mid', label='Varianza Acumulada')
