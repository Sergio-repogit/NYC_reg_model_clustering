...
from utils.helpers import print_custom_report, setup_logging, timer
...
def create_domain_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    df["is_new_listing"] = (
        df["number_of_reviews"].isna() | df["last_review"].isna()
    ).astype(int)
    ...
    if "latitude" in df.columns and "longitude" in df.columns:
        for name, coords in points.items():
            df[f"dist_{name}"] = haversine_np(
                df["latitude"], df["longitude"], coords[0], coords[1]
            )
...
def apply_pca_with_report(X: pd.DataFrame, n_components: float = 0.95):
    ...
    df_pca = pd.DataFrame(
        X_pca, columns=[f"PC{i + 1}" for i in range(X_pca.shape[1])], index=X.index
    )
    return df_pca, pca
...
def complete_feature_engineering(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    if "last_review" in df_fe.columns:
        ...
        df_fe["days_since_review"] = df_fe["days_since_review"].fillna(
            df_fe["days_since_review"].median()
        )
