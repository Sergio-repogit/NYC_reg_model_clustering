...
def calculate_gap_statistic(
    X: np.ndarray, n_clusters: int, n_references: int = 5
) -> float:
    ...
    for _ in range(n_references):
        ...
        kmeans_ref = KMeans(
            n_clusters=n_clusters, random_state=RANDOM_STATE, n_init=5
        ).fit(X_random)
...
def validate_kmeans_strategy(
    X: pd.DataFrame, k_range: list[int] = [2, 3, 4, 5, 6, 7, 8, 9, 10]
    ...
    consensus_k = int(
        np.median(
            [k_elbow, df_metrics["Silhouette"].idxmax(), df_metrics["Gap"].idxmax()]
        )
    )
